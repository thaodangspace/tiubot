...
Bot Slack giúp nhập chi tiêu vào Google Sheets một cách nhanh chóng, chạy trên
Deno.
...
- Tùy chọn: Kết nối OpenRouter để AI giúp hiểu cú pháp tự do hơn và trả lời vui
  tính hơn.
...
- Cấp quyền `chat:write` và `messages:read` (**Subscribe to events** ->
  `message.channels`).
...
Nếu muốn bật AI OpenRouter:

- `OPENROUTER_API_KEY`: API key tạo từ [OpenRouter](https://openrouter.ai/keys).
- (Tùy chọn) `OPENROUTER_MODEL`: Tên model, mặc định `gpt-4o-mini`.
- (Tùy chọn) `OPENROUTER_BASE_URL`: Endpoint API (giữ nguyên nếu dùng
  openrouter.ai).
- (Tùy chọn) `OPENROUTER_REFERER`: URL tham chiếu để OpenRouter xác định ứng
  dụng.
- (Tùy chọn) `OPENROUTER_APP_NAME`: Nhãn ứng dụng hiển thị trong dashboard.
...
Gửi tin nhắn trong channel mà bot được mời vào theo cú pháp:
`[Tên chi tiêu] [Số tiền]`

- Ví dụ 1: `ăn trưa 45k`
- Ví dụ 2: `siêu thị 1.2tr`
- Ví dụ 3: `xăng xe 50000`

> Khi bật OpenRouter, người dùng có thể nhắn tự nhiên hơn (VD: “chi 250k cafe
> sáng với khách”) và bot sẽ nhờ AI diễn giải trước khi ghi vào Google Sheets,
> đồng thời trả lời lại với giọng điệu hài hước hơn.
